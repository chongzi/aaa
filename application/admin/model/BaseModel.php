...
class BaseModel extends Model
{
    ...
    */
     ...
     * @access public
     * @param $where array 搜索条件
     * @param $field string 要获取的字段，如'name'
     * @param $limit int 要获取的数据数量
     * @param $order string 排序字段，多个用英文逗号隔开，如'id desc,name asc'
     * @return array 符合条件的数据集
     */
    public function columnBy($where=null,$field,$limit='',$order='id desc')
    {
        $m = new $this->class();
        return $m->where($where)->limit($limit)->order($order)->column($field);
    }
